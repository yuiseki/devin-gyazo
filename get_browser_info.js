...
function getDebugPort() {
    try {
        ...
        const processes = execSync("ps aux | grep -v grep | grep chrome | grep remote-debugging-port").toString();
        ...
        if (error.message.includes('No active Chrome')) {
            console.error('Error: No Chrome instance found with remote debugging enabled');
        } else {
